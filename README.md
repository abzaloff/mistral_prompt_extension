# Mistral + Gemini + Groq + OpenRouter + LM Studio Prompt Extension

Extension for Forge/Stable Diffusion WebUI that generates prompts from images using current Mistral Large, Medium, Small, and Ministral 3B/8B/14B models, current Gemini Flash/Flash-Lite/Pro models, Groq (`qwen/qwen3.8-27b`), free multimodal OpenRouter models, or local LM Studio models through its OpenAI-compatible API.
...
- Supports Mistral, Gemini, Groq, OpenRouter, and LM Studio model selection from the same UI
...
- **OpenRouter API key is required for OpenRouter models**. Free-model availability, quotas, and data policies depend on OpenRouter and the routed provider.
...
4. Set:
   ...
   - `OpenRouter API Key` (required for OpenRouter models; get one at https://openrouter.ai/settings/keys)
...
- OpenRouter choices: `inclusionai/ling-3.0-flash-vl:free` and `google/gemma-4-31b-it:free`, supporting text and image input through `/api/v1/chat/completions`.
